# 🖨️ Block of F and Big C

This program uses basic standard output statements to draw standard pixel-style letter shapes in the console.

## 📄 Source File
- [`simple_printf.c`](simple_printf.c)

## 🚀 Compilation & Execution
```powershell
gcc simple_printf.c -o simple_printf
.\simple_printf
```

## 📊 Expected Output Results
```text
######
#
#
#####
#
#
#



   ######
 ##      ##
 #
 #
 #
 #
 #
 ##      ##
   ######
```

## 🔍 Code Explanation
Draws static block shapes:
- **Block F**: Formed by printing a top horizontal bar (`######`), left vertical side rails (`#`), a middle horizontal bar (`#####`), and remaining bottom rails.
- **Big C**: Formed using carefully spaced spaces and `#` boundaries to shape a curved look.
